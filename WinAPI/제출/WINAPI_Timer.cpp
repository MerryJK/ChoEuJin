...
LRESULT CALLBACK WndProc(HWND hWnd, UINT iMessage, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_PAINT:
		...
		MoveToEx(hdc, xCenter, yCenter, NULL);
		LineTo(hdc, hX, hY);
		//Rectangle(hdc, 100, 100, 400, 400);		//1¹ø
...
void CALLBACK TimeProc(HWND hWnd, UINT uMsg, UINT idEvent, DWORD dwTime)
{
	...
	mAngle = M * (360 / 60)-90;
	hAngle = H * (360 / 12)-90;
	sX = xCenter + (cos(sAngle*3.14 / 180))*60;
	sY = yCenter + (sin(sAngle*3.14 / 180))*60;
	mX = xCenter + (cos(mAngle*3.14 / 180)) *70;
	mY = yCenter + (sin(mAngle*3.14 / 180))*70;
	hX = xCenter + (cos(hAngle*3.14 / 180))*50;
	hY = yCenter + (sin(hAngle*3.14 / 180))*50;
	InvalidateRect(hWnd, NULL, TRUE);
}